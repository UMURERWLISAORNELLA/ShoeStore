...
const MtnMomoPayment = ({ order, onPaymentSuccess, onPaymentError }) => {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const serverMessage = error.responseData?.details || error.responseData?.error || error.message || 'Payment processing failed';
      onPaymentError(typeof serverMessage === 'string' ? serverMessage : JSON.stringify(serverMessage));
        ...
        {!paymentStatus && (
          <>
            ...
            <Button
              ...
              className="w-full bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-6 text-lg"
            >
              {isProcessing ? (
                <>
